...
//#include <fftw3.h>
#include "/home/ayush/src/fftw-3.3.3/api/fftw3.h"
#include <pthread.h>
...
static image_list_t *create_gabor(int nscales, const int *orr, int width, int height)
{
    int i, j, fn;
    ...
    int nfilters = 0;
    for(i=0;i<nscales;i++)  nfilters+=orr[i];
    ...
    for(i = 1; i <= nscales; i++)
    {
        for(j = 1; j <= orr[i-1]; j++)
        {
            ...
            param[l][2] = 16*pow(orr[i-1], 2)/pow(32, 2);
            param[l][3] = M_PI/(orr[i-1])*(j-1);
            l++;
        }
    }
